...
describe('Voting Contract', () => {
  ...
  it('assigns the deployer as owner', async () => {
    ...
    assert.equal(owner.toLowerCase(), accounts[0].toLowerCase());
  });
  ...
  it('allows the owner to add a candidate', async () => {
    await voting.methods.addCandidate('Dave').send({ from: accounts[0], gas: '1000000' });
    const candidateCount = await voting.methods.getCandidateCount().call();
    assert.equal(candidateCount, 4);

    const added = await voting.methods.candidates(3).call();
    assert.equal(added[0], 'Dave');
  });

  it('does not allow non-owner to add candidates', async () => {
    let thrown = false;
    try {
      await voting.methods.addCandidate('Mallory').send({ from: accounts[1], gas: '1000000' });
    } catch (err) {
      thrown = true;
    }
  ...
  it('allows one vote per address and records which candidate', async () => {
    ...
    const voteIndex = await voting.methods.voteIndex(accounts[1]).call();
    assert.equal(voteIndex, '1');
    ...
    try {
      await voting.methods.vote(2).send({ from: accounts[1], gas: '1000000' });
    } catch (err) {
      throwCaught = true;
    }
